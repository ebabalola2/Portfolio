...
boggle_t * bg_init(int seed, char * dict_fn){
  ...
  FILE * file = fopen(dict_fn, "r");
  while(fscanf(file, "%1023s\n", words) != EOF)
  {
    for(char *f = words; (*f =toupper(*f)) != 0; f++);
    hm_add(bg->dict,words);
  }
  fclose(file);
  return bg;
}
...
llist_t * bg_all_words(boggle_t *bg){
  ...
  llist_t * made_words = ll_init();
  for(int i = 0; i < hm->num_buckets; i++)
  {
    ll_node_t * traverse = hm->buckets[i]->head;

    while(traverse != NULL)
    {
      ll_push(made_words, traverse->val);
  ...
  return made_words; //clearly, this won't be null when your done.
}
...
// #1
  ...
  if(r<0 || r >= BOGGLE_DIMENSION)
  {
    return;
  }

  if(c<0 || c >= BOGGLE_DIMENSION)
  {
    return;
  }
  ...
  char cLetter = bg->board[r][c];

  if(cLetter == 'Q')
  {
    cur_word[strlen(cur_word)] = cLetter;
...
bool check_word = hm_check(bg->dict, cur_word);

bool existing_word = hm_check(hm,cur_word);

if(check_word && !existing_word && strlen(cur_word) >= 3)
{
  hm_add(hm, cur_word);
}
...
visited[r][c] = 1;
  ...
  //printf("%s\n", cur_word);
  //printf("%d\n", visited[BOGGLE_DIMENSION][BOGGLE_DIMENSION]);
}
