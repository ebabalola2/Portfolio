...
public class Creature {
    ...
    public void bigStep(int[] x, int [] y)
    {
        int oldx = this.point.x;
        int oldy = this.point.y;
        GridPoint g = new GridPoint(oldx, oldy);

        this.point.x = (this.point.x + x[dir] + City.HEIGHT) % City.HEIGHT;
        this.point.y =(this.point.y + y[dir] + City.WIDTH) % City.WIDTH;
        this.city.updateGrid(this, g,point);
    }
    public int guageDistance(GridPoint creatureP, GridPoint preyP, int[] r, int[] c) {
        int d = creatureP.dist(preyP);
        int pushD = 0;
        for (int i = 0;i<NUM_DIRS;++i) {
          GridPoint p = new GridPoint(creatureP.x, creatureP.y);
          p.x = (this.point.x + r[i] + City.HEIGHT) % City.HEIGHT;
          p.y = (this.point.y + c[i] + City.WIDTH) % City.WIDTH;
          if (p.dist(preyP) == 0) {
            pushD = i;
            break;
